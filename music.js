(() => {
  ...
  theme.textContent = `
    :root{
      --ink:#2b0910!important;--cream:#fff6e4!important;--gold:#e6c979!important;--soft:#e7d8bd!important;--line:rgba(230,201,121,.34)!important;
    ...
    *,button,a,input,textarea,select{font-family:'Italianno','Brush Script MT',cursive!important}
    html,body{background:#2b0910!important}
    body,.theatre{
      background:
        radial-gradient(circle at 50% 18%,rgba(142,49,53,.30),transparent 34%),
        radial-gradient(ellipse at 50% 44%,#5a1720 0%,#451018 37%,#310b12 68%,#1d070b 100%)!important
    }
    .silk{background:repeating-linear-gradient(95deg,transparent 0%,rgba(229,191,101,.11) 9%,rgba(33,4,9,.42) 15%,rgba(255,220,137,.09) 19%,rgba(26,3,8,.68) 24%)!important}
    .secondary{background:#351018dd!important}.secondary:hover{background:#5a1720!important}
    .portrait-arch{background:#451018!important}
    .portrait-shade{background:linear-gradient(transparent 60%,rgba(43,9,16,.84))!important}
    .venue-image{background:linear-gradient(rgba(43,9,16,.48),rgba(29,7,11,.76)),url('./assets/cards/venue.webp') center 45%/cover!important}
    .noscript{background:#2b0910!important}
    ...
    .god-intro{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:32px;background:
      radial-gradient(circle at 50% 24%,rgba(139,52,55,.42),transparent 32%),
      radial-gradient(ellipse at 50% 52%,#5a1720 0%,#3a0f17 58%,#21070c 100%);
      overflow:hidden;text-align:center;color:#fff6e4;border:1px solid rgba(230,201,121,.5)}
    .god-intro::before,.god-intro::after{content:'';position:absolute;width:42vw;height:100vh;top:-10vh;background:linear-gradient(180deg,rgba(255,237,188,.30),rgba(230,201,121,.08) 42%,transparent 82%);clip-path:polygon(48% 0,52% 0,100% 100%,0 100%);filter:blur(12px);opacity:.35;pointer-events:none}
    .god-intro::before{left:-24vw;transform:rotate(12deg)}.god-intro::after{right:-24vw;transform:rotate(-12deg)}
    .god-card{position:relative;z-index:2;width:min(760px,92vw);padding:28px 18px}
    .god-cross{font-size:56px;line-height:1;color:#e6c979;text-shadow:0 0 24px rgba(230,201,121,.36);margin-bottom:14px}
    .god-kicker{font-size:34px;color:#e6c979;margin-bottom:10px}
    .god-title{font-size:clamp(64px,12vw,106px);line-height:.95;margin:0 0 20px;color:#fff6e4;text-shadow:0 4px 22px rgba(0,0,0,.28)}
    .god-copy{font-size:clamp(30px,5.7vw,46px);line-height:1.28;max-width:680px;margin:0 auto 28px;color:#fff6e4}
    .god-enter{border:1px solid rgba(230,201,121,.78);background:rgba(79,18,28,.58);color:#f7dfa2;border-radius:999px;padding:12px 26px;font-size:32px;box-shadow:0 0 30px rgba(230,201,121,.11);backdrop-filter:blur(8px)}
    ...
    @media(max-width:650px){
      ...
      .god-intro{padding:22px}.god-cross{font-size:46px}.god-kicker{font-size:29px}.god-copy{font-size:clamp(28px,7.4vw,38px)}.god-enter{font-size:29px}
    }
  `;
  ...
  ['open','skip'].forEach(id => document.getElementById(id)?.addEventListener('click', () => {
    if (!started && !requested) play();
  }));
